...
// G, T or P.
func CustomBodyLimit(limit string) echo.MiddlewareFunc {
	...
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c echo.Context) error {
			...
			if req.ContentLength > config.limit {
				return c.HTML(http.StatusRequestEntityTooLarge, "<div>File too large. Must be less than 2MB</div>")
			}
...
func (r *limitedReader) Read(b []byte) (n int, err error) {
	...
	r.read += int64(n)
	if r.read > r.limit {
		return n, fmt.Errorf("<div>File too large. Must be less than 2MB</div>")
	}
	return
}
